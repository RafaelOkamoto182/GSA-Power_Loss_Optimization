...
%     iteration
    ...
    %Evaluation of agents. Fitness contem o valor de fit para cada soluçao em X.
    fitness=evaluateF(X,F_index, mpc);

    if min_flag==1
    [best best_X]=min(fitness); %minimization. best é o valor do melhor fit; best_X é o index do melhor fit
    else
    [best best_X]=max(fitness); %maximization.
    end

    %FBest é o melhor valor de fit e LBest é a melhor posicao pra esse valor, ou seja, a melhor solução.
    if iteration==1
       Fbest=best;
       Lbest=X(best_X,:);
    end
